...
<div class="table-container body">
        <h2>Let's Talk  Table</h2>
        <?php
        include('config.php');

        // Fetch data from the database
        $sql = "SELECT id, name, email, phone, message FROM lettaalk"; // Added `id` field
        $stmt = $conn->query($sql);
            ...
            <tbody>
                <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): ?>
                    <tr>
                        ...
                        <td>
                            <a href="edit.php?id=<?= urlencode( base64_encode(string: $row['id'])); ?>"
                                class="btn btn-success btn-edit btn-sm">Edit</a>
                            <a class="btn btn-sm btn-danger"
                                href="delete.php?id=<?= urlencode(base64_encode($row['id'])); ?>"
